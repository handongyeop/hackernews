...
function applyApiMixin(targetClass, baseClasses) {
...
function newsFeed() {
  ...
  template = template.replace("{{__next_page__}}", String(newsFeed.length / (store.currentPage * 10) > 1 ? store.currentPage + 1 : store.currentPage));
  updateView(template);
}
